...
final class OutfitRepository {
    ...
    var currentWeather: WeatherContext? {
        if !sessionOutfits.isEmpty, let weather = sessionWeather {
            print("🌤️ [OutfitRepo] currentWeather → session: \(Int(weather.tempFahrenheit))°F, \(weather.condition)")
            return weather
        } else if let weather = preGeneratedWeather {
            let ageMinutes = preGenCacheTimestamp.map { Int(-$0.timeIntervalSinceNow / 60) } ?? -1
            print("🌤️ [OutfitRepo] currentWeather → pre-generated: \(Int(weather.tempFahrenheit))°F, \(weather.condition) (cached \(ageMinutes) min ago)")
            return weather
        }
        print("🌤️ [OutfitRepo] currentWeather → nil (no weather available)")
        return nil
    }
        ...
        do {
            if let preGen = try await api.getPreGeneratedOutfits() {
                ...
                if preGen.fallback == true {
                    ...
                    if let weather = preGen.weather {
                        print("🌤️ [OutfitRepo] 📦 Pre-gen weather: \(Int(weather.tempFahrenheit))°F, \(weather.condition) (generated by backend overnight)")
                    }
